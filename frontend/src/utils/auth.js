...
export const login = async (email, password, rememberMe = false) => {
    try {
        ...
        if (status === 200) {
            setAuthUser(data.access, data.refresh, rememberMe);
            Toast.fire({ icon: "success", title: "Signed in successfully" });
            return { success: true, error: null, fieldErrors: {} };
        }
    } catch (error) {
...
// Enhanced register function with better error handling
export const register = async (full_name, email, password, password2, rememberMe = false) => {
    try {
        const { data } = await axios.post("user/register/", {
            full_name,
            email,
...
export const setUser = async () => {
    ...
    if (isAccessTokenExpired(accessToken)) {
        const response = await getRefreshToken(refreshToken);
        setAuthUser(response.access, response.refresh, rememberMe);
    } else {
        setAuthUser(accessToken, refreshToken, rememberMe);
    }
};

// Function to set the authenticated user and update user state with remember me option
export const setAuthUser = (access_token, refresh_token, rememberMe = false) => {
    // Set remember_me cookie based on user preference
    Cookies.set("remember_me", rememberMe.toString(), {
        expires: rememberMe ? 30 : undefined, // 30 days if remember me, session if not
        secure: true,
        sameSite: 'Strict'
    });
    ...
    if (rememberMe) {
        accessTokenOptions.expires = 1; // 1 day
        refreshTokenOptions.expires = 7; // 7 days
    }
...
// Google OAuth functions with remember me option
export const handleGoogleLogin = async (credential, rememberMe = false) => {
    try {
        const response = await axios.post("user/google-login/", {
            credential: credential
        });
        ...
        if (response.data.success) {
            setAuthUser(response.data.access, response.data.refresh, rememberMe);
            Toast.fire({ icon: "success", title: "Google login successful!" });
            return { success: true, error: null, fieldErrors: {} };
        } else {
            return { 
...
export const handleGoogleRegister = async (credential, rememberMe = false) => {
    try {
        const response = await axios.post("user/google-register/", {
            credential: credential
        });
        ...
        if (response.data.success) {
            // Automatically log in after successful registration
            const loginResponse = await handleGoogleLogin(credential, rememberMe);
            if (loginResponse.success) {
                return { success: true, error: null, fieldErrors: {} };
            }
            return loginResponse;
...
// Function to clear authentication tokens while keeping remember me preference
