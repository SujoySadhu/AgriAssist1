...
function MyAccount() {
    ...
    const axiosInstance = useAxios();
    const { t } = useLanguage(); // Language context hook
    ...
    const handleProcessDoc = async () => {
        if (!docFile) {
            setDocStatus(t('selectDocFile'));
            return;
        }
        
        setDocStatus(t('processingDoc'));
        const formData = new FormData();
        formData.append("documents", docFile);
        ...
        try {
            ...
            setDocStatus(res.data.message || t('docProcessedSuccess'));
            setDocFile(null);
        } catch (err) {
            setDocStatus(t('docProcessError'));
        }
    };
                        ...
                        <div className="card mb-4 shadow-sm">
                            <div className="card-body">
                                <div className="d-flex align-items-center">
                                    <img 
                                        src={profileData?.image || `${import.meta.env.VITE_BACKEND_URL || 'http://127.0.0.1:8000'}/media/default/default-user.jpg`}
                                        alt={profileData?.full_name || t('user')} 
                                        className="rounded-circle me-4 border"
                                        style={{ 
                                            width: "100px", 
                                            height: "100px", 
                                        ...
                                        <div className="col-md-6">
                                            <div className="card h-100 border-0 shadow-sm">
                                                <div className="card-body">
                                                    ...
                                                    <button 
                                                        className="btn btn-outline-success w-100"
                                                        onClick={() => Toast.info(t('userManagementComing'))}
                                                    >
                                                        <i className="fas fa-cog me-2"></i>
                                                        {t('manageUsers')}
                                                    </button>
                        ...
                        <div className="row g-4">
                            ...
                            <div className="col-lg-6">
                                <div className="card border-0 shadow-sm h-100">
                                    <div className="card-header bg-white border-bottom">
                                        <div className="d-flex justify-content-between align-items-center">
                                            <h5 className="mb-0">{t('latestPosts')}</h5>
                                            <Link to="/posts/" className="btn btn-sm btn-outline-primary">
                                                {t('viewAll')}
                                            </Link>
                                        </div>
                                            ...
                                            {noti?.slice(0, 5)?.map((n, index) => (
                                                <div className="list-group-item border-0 px-0 py-3" key={index}>
                                                    <div className="d-flex align-items-start">
                                                        ...
                                                        <div className="flex-grow-1">
                                                            <h6 className="mb-1">{t(n.type.toLowerCase())}</h6>
                                                            <p className="mb-1 small">
                                                                {n.type === "Like" && t('notificationLike', { title: `${n?.post?.title?.slice(0, 30)}...` })}
                                                                {n.type === "Comment" && t('notificationComment', { title: `${n?.post?.title?.slice(0, 30)}...` })}
                                                                {n.type === "Bookmark" && t('notificationBookmark', { title: `${n?.post?.title?.slice(0, 30)}...` })}
                                                            </p>
                                                            <small className="text-muted">
                                                                <i className="fas fa-clock me-1"></i>
                                                                {moment(n.date).fromNow()}
                ...
                <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                <div className="mb-3">
                                    <label className="form-label">{t('selectDocFile')}</label>
                                    <input 
                                        type="file" 
                                        accept=".docx" 
                                        onChange={handleDocChange} 
